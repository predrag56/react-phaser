...
const Zone = TYPES.ZONE; // export const Graphics = TYPES.GRAPHICS;
...
class Scene$1 extends Phaser.Scene {
  constructor(props) {
    const _props$assets = props.assets,
    ...
    const sceneProps = lodash_2(parsedProps, SCENE_HOOKS);
    sceneProps.key = props.name;
    const hooks = lodash_1(parsedProps, SCENE_HOOKS);
    super(sceneProps);
    this.key = '';
    this[POOL] = [];
  ...
  preload(...args) {
    ...
    const preload = this[HOOKS].preload;
  ...
  create(...args) {
    ...
    const create = this[HOOKS].create;
...
class GameObject {
  ...
  update(newProps, oldProps) {
    const props = _objectSpread({}, lodash_1(defaultProps, this.allowedProps), {}, this.defaultProps, {}, lodash_1(newProps, this.allowedProps), {}, lodash_1(newProps, eventNames));
...
const allowedProps$1 = ['x', 'y', 'z', 'width', 'height', 'alpha', 'angle', 'scale', 'tint', 'visible'];
...
const allowedProps$2 = ['texture', 'frame', 'x', 'y', 'z', 'alpha', 'angle', 'scale', 'tint', 'visible', 'origin', 'interactive', // 'animations', TODO or leave writeonceonly
'startFrame', 'play'];
const transitionProps$1 = ['x', 'y'];
...
class Sprite$1 extends GameObject {
  ...
  prepareAnimationConfig(_ref) {
    let frames = _ref.frames,
        generateFrameNames = _ref.generateFrameNames,
        generateFrameNumbers = _ref.generateFrameNumbers,
        config = _objectWithoutProperties(_ref, ["frames", "generateFrameNames", "generateFrameNumbers"]);
...
const allowedProps$3 = ['texture', 'frame', 'x', 'y', 'z', 'width', 'height', 'origin', 'alpha', 'angle', 'scale', 'tint', 'visible', 'interactive'];
...
const allowedProps$4 = ['play', 'pause', 'marker', 'mute', 'volume', 'rate', 'detune', 'seek', 'loop', 'delay'];
...
const _Phaser$GameObjects = Phaser.GameObjects,
      Components = _Phaser$GameObjects.Components,
      GameObject$1 = _Phaser$GameObjects.GameObject;
const getTintAppendFloatAlpha = Phaser.Renderer.WebGL.Utils.getTintAppendFloatAlpha;
...
class VideoGameObject extends GameObject$1 {
  ...
  createVideoElement(config = emptyObject) {
    ...
    for (let key in elementProperties) {
      let value = lodash_3(config, key, elementProperties[key]);
    ...
    const _config$playbackTimeC = config.playbackTimeChangeEventEnable,
...
const allowedProps$5 = ['x', 'y', 'z', 'alpha', 'angle', 'scale', 'tint', 'visible', 'origin', 'mute', 'volume', 'play', 'src', 'controls', 'loop', 'muted', 'playsInline', 'crossOrigin', 'playbackTimeChangeEventEnable'];
...
const allowedProps$7 = ['x', 'y', 'key', 'children', 'fontSize', 'font', 'interactive', 'width', 'height', 'align', 'origin'];
...
const allowedProps$8 = ['x', 'y', 'width', 'height', 'interactive'];
...
function VideoCanvasCreator(config, addToScene) {
  ...
  const gameObject = new VideoGameObject(this.scene, 0, 0, width, height, config);
  BuildGameObject(this.scene, gameObject, config);
  return gameObject;
}

function VideoCanvasFactory(config) {
  const gameObject = new VideoGameObject(this.scene, config);
...
var _jsxFileName = "/Users/firanolfind/Work/Ainulindale/React-Phaser-Binding/dev/src/Game.js";
...
class GameContext extends React.Component {
  componentDidMount() {
    ...
    const game = new Phaser.Game(_objectSpread$2({
      type: Phaser.AUTO
    }, config, {
      autoStart: false,
      parent: this[tagRef],
