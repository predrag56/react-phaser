...
const version="1.3.5";const name="react-phaser-bindings";
...
const uid = new shortUniqueId_min();
...
const UPDATE = Symbol('Scene.update');
...
const insertBeforeToScene = (scene, child, beforeChild) => {
  addToScene(scene, child);
  insertBefore(scene.sys.displayList.list, child.instance, beforeChild.instance);
  return child;
};
class Scene extends Phaser$1.Scene {
  constructor(props) {
    const sceneProps = lodash_2(props, SCENE_HOOKS, 'assets', 'children');
    sceneProps.key = props.name;
    const hooks = lodash_1(props, SCENE_HOOKS);
    super(sceneProps);
    ...
    _defineProperty(this, IS_REGISTERED, false);

    this.key = props.name;
    this[ASSETS] = props.assets;
    this[HOOKS] = lodash_1(hooks, SCENE_HOOKS);
    this[PROPS] = props;
  }
  ...
  [UPDATE](newProps, oldProps) {
    ...
    this[HOOKS] = lodash_1(newProps, SCENE_HOOKS);
    this[PROPS] = newProps;
  }
...
class GameObject {
  constructor(props) {
    ...
    _defineProperty(this, "scenePool", []);

    _defineProperty(this, "transitionsPool", []);
    ...
    this.fullEventMap = { ...defaultEventMap,
      ...this.eventMap
    };
    this.allowedDefaultProps = { ...lodash_1(defaultProps, this.allowedProps),
      ...this.defaultProps
    };
  ...
  register() {
    invariant_1(`Register method of ${this} should be overwritten`);
  }
  ...
  update(newProps, oldProps) {
    this.props = { ...this.allowedDefaultProps,
      ...newProps
    };
    ...
    const props = { ...this.allowedDefaultProps,
      ...lodash_1(newProps, this.allowedProps),
      ...lodash_1(newProps, this.eventNames)
    };
  ...
  parseTransitionsProps() {
    ...
    if (typeof transition === 'string') {
      return transition.split(',').map(elem => elem.trim().split(' ').map(el => el.trim())).reduce((acc, [key, ...params]) => {
        acc[key] = params;
        return acc;
      }, {});
    }
...
const setFillStyle = (inst, {
  fillColor,
  alpha
}) => inst.setFillStyle(fillColor, alpha);
const setStrokeStyle = (inst, {
  strokeWidth,
  strokeColor,
  strokeAlpha
...
const Scene$1 = TYPES.SCENE;
const Container = TYPES.CONTAINER;
const Group = TYPES.GROUP;
const Image = TYPES.IMAGE;
const Sprite = TYPES.SPRITE;
const Text = TYPES.TEXT;
const BitmapText = TYPES.BITMAPTEXT;
const CounterText = TYPES.COUNTERTEXT;
const CounterBitmapText = TYPES.COUNTERBITMAPTEXT;
const Tween = TYPES.TWEEN;
const Audio = TYPES.AUDIO;
const Video = TYPES.VIDEO;
const Zone = TYPES.ZONE;
const Particles = TYPES.PARTICLES;
const Blitter = TYPES.BLITTER;
const Input = TYPES.INPUT;
const Mask = TYPES.MASK;
const Circle = TYPES.CIRCLE;
const Ellipse = TYPES.ELLIPSE;
const Rectangle = TYPES.RECTANGLE;
const RoundedRectangle = TYPES.ROUNDEDRECTANGLE;
const Triangle = TYPES.TRIANGLE;
const Polygon = TYPES.POLYGON;
const Star = TYPES.STAR; // export const Graphics = TYPES.GRAPHICS;

const allowedProps$1 = ['play', 'pause', 'marker', 'mute', 'volume', 'rate', 'detune', 'seek', 'loop', 'delay'];
const performedProps$2 = {
  play: (inst, {
    play,
    marker
  }) => {
  ...
  delay: (inst, {
    delay
  }) => inst.audioPlayDelay(delay)
};

class Audio$1 extends GameObject {
  register(scene) {
    const {
      name
    } = this.props;
...
const performedProps$3 = {};
const allowedProps$2 = ['x', 'y', 'z', 'width', 'height', 'alpha', 'angle', 'scale', 'tint', 'visible'];

class Container$1 extends GameObject {
  register(scene) {
    const {
      x,
      y
...
const allowedProps$3 = ['texture', 'frame', 'x', 'y', 'z', 'alpha', 'angle', 'scale', 'tint', 'visible', 'origin', 'interactive', // 'animations', TODO or leave writeonceonly
'startFrame', 'play'];
const transitionProps$1 = ['x', 'y', 'alpha', 'angle', 'scale', 'tint'];
const performedProps$4 = {
  frame: frame,
  texture: texture,
  width,
  height,
...
class Sprite$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
  ...
  registerAnimations() {
    ...
    for (const config of animations) {
      ...
      if (key) {
        ...
        anim = new Phaser$1.Animations.Animation(anims, key, this.prepareAnimationConfig(config));
...
const allowedProps$4 = ['texture', 'frame', 'x', 'y', 'z', 'width', 'height', 'origin', 'alpha', 'angle', 'scale', 'tint', 'visible', 'interactive', 'mask'];
const performedProps$5 = {
  interactive,
  frame: frame,
  texture: texture,
  origin,
...
const transitionProps$2 = ['x', 'y', 'alpha', 'angle', 'scale', 'tint'];

class Image$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
Object.assign(Image$1.prototype, {
  type: TYPES.IMAGE,
  transitionProps: transitionProps$2,
  performedProps: performedProps$5,
  allowedProps: allowedProps$4
});

const allowedProps$5 = ['x', 'y', 'z', 'texture', 'alpha', 'angle', 'scale', 'tint', 'visible', 'origin', 'mute', 'volume', 'play', 'src', 'controls', 'loop', 'muted', 'playsInline', 'crossOrigin', 'playbackTimeChangeEventEnable', 'exit'];
const eventMap$1 = {
  onPlay: 'play',
  onStop: 'stop',
  onCreated: 'created',
...
class Video$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const allowedProps$6 = ['children', 'x', 'y', 'z', 'originX', 'originY', 'alpha', 'angle', 'scale', 'tint', 'visible', 'origin', 'interactive'];
const performedProps$7 = {
  width,
  height,
...
class Text$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const allowedProps$7 = ['x', 'y', 'key', 'children', 'fontSize', 'font', 'interactive', 'width', 'height', 'align', 'origin', 'visible'];
const performedProps$8 = {
  children: textChildren,
  fontSize,
  font: font,
  width,
...
class BitmapText$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const allowedProps$8 = ['from', 'to', 'duration', 'delay', 'play'];
const CounterMixin = {
  defaultProps: {
    locale: undefined,
    pattern: undefined,
...
const allowedProps$9 = [...allowedProps$6, ...allowedProps$8].filter(key => key !== 'children');
const performedProps$a = { ...lodash_2(performedProps$7, 'children'),
  ...performedProps$9
};

class CounterText$1 extends Text$1 {
  constructor(..._args) {
    super(..._args);

    _defineProperty(this, "handleOnUpdate", (tween, {
      value
    }) => {
    ...
    _defineProperty(this, "handleOnComplete", (...args) => {
      const {
        onComplete
      } = this.props;
...
const allowedProps$a = [...allowedProps$7, ...allowedProps$8].filter(key => key !== 'children');
const performedProps$b = { ...lodash_2(performedProps$8, 'children'),
  ...performedProps$9
};

class CounterText$2 extends BitmapText$1 {
  constructor(..._args) {
    super(..._args);
...
const allowedProps$b = ['x', 'y', 'z', 'radius', 'fillColor', 'alpha', 'strokeWidth', 'strokeColor', 'strokeAlpha', 'origin', 'scale', 'visible', 'interactive', 'stroke'];
const performedProps$c = {
  interactive,
...
class Circle$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const allowedProps$c = ['x', 'y', 'z', 'width', 'height', 'fillColor', 'alpha', 'origin', 'angle', 'strokeWidth', 'strokeColor', 'strokeAlpha', 'smoothness', 'scale', 'visible', 'interactive', 'stroke'];
const performedProps$d = {
  interactive,
  width: size,
  height: size,
  fillColor: setFillStyle,
...
class Ellipse$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const allowedProps$d = ['x', 'y', 'z', 'x1', 'y1', 'x2', 'y2', 'x3', 'y3', 'fillColor', 'alpha', 'origin', 'angle', 'strokeWidth', 'strokeColor', 'strokeAlpha', 'smoothness', 'scale', 'visible', 'interactive', 'stroke'];
const performedProps$e = {
  interactive,
  fillColor: setFillStyle,
  origin,
  stroke
...
class Triangle$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const allowedProps$e = ['x', 'y', 'z', 'width', 'height', 'fillColor', 'alpha', 'angle', 'strokeWidth', 'strokeColor', 'strokeAlpha', 'smoothness', 'scale', 'origin', 'visible', 'interactive', 'stroke'];
const performedProps$f = {
  interactive,
  width: size,
  height: size,
  fillColor: setFillStyle,
...
class Rectangle$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const allowedProps$f = ['x', 'y', 'z', 'points', 'fillColor', 'alpha', 'angle', 'origin', 'strokeWidth', 'strokeColor', 'strokeAlpha', 'smoothness', 'scale', 'visible', 'interactive', 'stroke'];
const performedProps$g = {
  interactive,
  fillColor: setFillStyle,
  origin,
  stroke,
...
class Polygon$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const allowedProps$g = ['x', 'y', 'z', 'points', 'innerRadius', 'outerRadius', 'fillColor', 'alpha', 'origin', 'angle', 'strokeWidth', 'strokeColor', 'strokeAlpha', 'smoothness', 'scale', 'visible', 'interactive', 'stroke'];
const performedProps$h = {
  interactive,
  fillColor: setFillStyle,
  origin,
  stroke
...
class Star$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const performedProps$i = {
  interactive,
  origin // TODO: check drop zone more detail later
...
const allowedProps$h = ['x', 'y', 'width', 'height', 'interactive', 'origin'];

class Zone$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
class AnimatedParticle extends Particle {
  constructor(scene, animationName, emitter) {
    ...
    this.i = 0;
    this.scene = scene;
    this.anims = new Phaser$1.GameObjects.Components.Animation(this);
...
const allowedProps$i = ['texture', 'frame', 'pause', 'start'];
const performedProps$j = {
  texture: texture,
  frame: (inst, {
    frame
  }, {
...
class Particles$1 extends GameObject {
  register(scene) {
    const {
      texture,
      animation
  ...
  update(newProps = emptyObject_1, oldProps = emptyObject_1) {
    oldProps = oldProps || {};
...
const allowedProps$j = ['play'];
const performedProps$k = {
  play: (inst, {
    play
  }, object) => {
    if (play) {
...
class Tween$1 extends Transparent {
  ...
  prepareConfig(configs) {
    return Object.entries(configs).reduce((acc, [key, config]) => {
      const conf = lodash_2(config, 'repeat');
  ...
  startTween(key) {
    ...
    const tween = scene.add.tween({ ...config,
      props,
...
const allowedProps$k = ['x', 'y', 'frame', 'data'];
const performedProps$l = {
  data: (inst, {
    data
  }) => {
    inst.clear();
...
class Blitter$1 extends GameObject {
  register(scene) {
    const {
      x,
      y,
...
const stopPropagationTouchEvents = element => {
...
class InputText extends DOMElement {
  constructor(scene, config = emptyObject_1) {
    config = { ...config
    };
    const {
      x = 0,
    ...
    for (const key in elementProperties) {
      const value = lodash_3(config, key, elementProperties[key]);
    ...
    for (const key in styleProperties) {
      ...
      const value = lodash_3(style, key, prop[1]);
  ...
  set style(styleProps = emptyObject_1) {
    if (!this.node) return;
...
const allowedProps$l = ['x', 'y', 'width', 'height', 'placeholder', 'value', 'readonly', 'style', 'tooltip'];
const eventMap$2 = {
  onInput: 'input',
  onChange: 'change',
  onDbClick: 'dblclick',
...
class Input$1 extends GameObject {
  register(scene) {
    this.scene = scene;
...
Object.assign(Input$1.prototype, {
  type: TYPES.INPUT,
  allowedProps: allowedProps$l,
  eventMap: eventMap$2
});

const maskProps = ['x', 'y', 'z', 'scale', 'inverse'];
const performedProps$m = {
  inverse: (shape, mask, {
    inverse
  }) => {
...
class Mask$1 extends Transparent {
  postRegister() {
  ...
  createShape() {
    ...
    invariant_1(this.shape, `React-Phaser-Bindings: Unsupported Mask type: ${type}`);
  }
  ...
  updateShape(newProps, oldProps) {
    ...
    const props = lodash_1(newProps, maskProps);
  ...
  update(newProps, oldProps) {
    if (!this.registered) return;
    const pureNewProps = lodash_2(newProps, maskProps, 'children', 'type', 'fillColor');
    const pureOldProps = lodash_2(oldProps, maskProps, 'children', 'type', 'fillColor');
    this.props = newProps;
...
var Components = {
  ...
  [TYPES.SPRITE]: Sprite$1,
  [TYPES.IMAGE]: Image$1,
  [TYPES.AUDIO]: Audio$1,
  [TYPES.VIDEO]: Video$1,
  [TYPES.TEXT]: Text$1,
  [TYPES.BITMAPTEXT]: BitmapText$1,
  [TYPES.COUNTERTEXT]: CounterText$1,
  [TYPES.COUNTERBITMAPTEXT]: CounterText$2,
  [TYPES.CIRCLE]: Circle$1,
  [TYPES.ELLIPSE]: Ellipse$1,
  [TYPES.TRIANGLE]: Triangle$1,
  [TYPES.RECTANGLE]: Rectangle$1,
  [TYPES.POLYGON]: Polygon$1,
  [TYPES.STAR]: Star$1,
  [TYPES.ZONE]: Zone$1,
  [TYPES.PARTICLES]: Particles$1,
  [TYPES.TWEEN]: Tween$1,
  [TYPES.BLITTER]: Blitter$1,
  [TYPES.INPUT]: Input$1,
  [TYPES.MASK]: Mask$1
};
...
const PhaserRenderer = reactReconciler({
  getChildHostContext() {
  },
  ...
  createInstance(type, props, scope) {
    ...
    invariant_1(Node, `React-Phaser-Bindings: Unsupported component type: ${type}`);
...
class InputTextPlugin extends Phaser$1.Plugins.BasePlugin {
...
const Game = React__default.forwardRef((props, ref) => {
  return React__default.createElement(GameContext, _extends({}, props, {
    forwardedRef: ref
  }));
});
