...
const version="1.1.0";const name="react-phaser-bindings";
...
var Input = TYPES.INPUT; // export const Graphics = TYPES.GRAPHICS;
...
var addToScene$1 = function addToScene(scene, child) {
...
function () {
  ...
  _createClass(GameObject, [{
    ...
    value: function update(newProps, oldProps) {
      var props = _objectSpread({}, lodash_1(defaultProps, this.allowedProps), {}, this.defaultProps, {}, lodash_1(newProps, this.allowedProps), {}, lodash_1(newProps, this.eventNames));
...
var allowedProps$2 = ['texture', 'frame', 'x', 'y', 'z', 'alpha', 'angle', 'scale', 'tint', 'visible', 'origin', 'interactive', // 'animations', TODO or leave writeonceonly
'startFrame', 'play'];
...
var allowedProps$3 = ['texture', 'frame', 'x', 'y', 'z', 'width', 'height', 'origin', 'alpha', 'angle', 'scale', 'tint', 'visible', 'interactive'];
...
var CanvasPool = Phaser$1.Display.Canvas.CanvasPool;
var _Phaser$GameObjects = Phaser$1.GameObjects,
    Components = _Phaser$GameObjects.Components,
    GameObject$1 = _Phaser$GameObjects.GameObject;
var getTintAppendFloatAlpha = Phaser$1.Renderer.WebGL.Utils.getTintAppendFloatAlpha;
...
function (_GameObject) {
  ...
  function VideoGameObject(scene, config) {
    ...
    _this.canvas = CanvasPool.create(_assertThisInitialized(_this), _this.resolution * width, _this.resolution * height);
  ...
  _createClass(VideoGameObject, [{
    ...
    value: function renderWebGL(renderer, src, interpolationPercentage, camera, parentMatrix) {
      ...
      var width = frame.width,
          height = frame.height;
      this.pipeline.batchTexture(src, frame.glTexture, width, height, src.x, src.y, width / src.resolution, height / src.resolution, src.scaleX, src.scaleY, src.rotation, src.flipX, src.flipY, src.scrollFactorX, src.scrollFactorY, src.displayOriginX, src.displayOriginY, 0, 0, width, height, getTintAppendFloatAlpha(src._tintTL, camera.alpha * src._alphaTL), getTintAppendFloatAlpha(src._tintTR, camera.alpha * src._alphaTR), getTintAppendFloatAlpha(src._tintBL, camera.alpha * src._alphaBL), getTintAppendFloatAlpha(src._tintBR, camera.alpha * src._alphaBR), src._isTinted && src.tintFill, 0, 0, camera, parentMatrix);
    }
  }, {
    key: "renderCanvas",
    ...
    value: function createVideoElement() {
      ...
      try {
        ...
        for (var _iterator = elementEvents[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          _loop();
        }
      } catch (err) {
        _didIteratorError = true;
    ...
    value: function load(src) {
      ...
      if (typeof src === 'object') {
        ...
        try {
          for (var _iterator2 = videoTypes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
...
Phaser$1.Class.mixin(VideoGameObject, [Components.Alpha, Components.BlendMode, Components.ComputedSize, Components.Crop, Components.Depth, Components.Flip, Components.GetBounds, Components.Mask, Components.Origin, Components.Pipeline, Components.ScrollFactor, Components.Tint, Components.Transform, Components.Visible]);

var allowedProps$5 = ['x', 'y', 'z', 'alpha', 'angle', 'scale', 'tint', 'visible', 'origin', 'mute', 'volume', 'play', 'src', 'controls', 'loop', 'muted', 'playsInline', 'crossOrigin', 'playbackTimeChangeEventEnable'];
...
var allowedProps$9 = ['texture', 'frame', 'pause', 'start'];
var performedProps$a = {
  texture: texture,
...
function (_GameObject) {
  ...
  _createClass(Particles, [{
    ...
    value: function update() {
      ...
      var newConfig = _objectSpread$2({}, newProps.config, {
        frame: newProps.frame,
        animation: undefined
      });

      var oldConfig = _objectSpread$2({}, oldProps.config, {
...
var allowedProps$a = ['play'];

var Tween$1 =
/*#__PURE__*/
function (_GameObject) {
  _inherits(Tween, _GameObject);
  ...
  _createClass(Tween, [{
    ...
    key: "registerChildren",
    value: function registerChildren() {
      var instancePool = this.instancePool,
          pool = this.pool,
          scenePool = this.scenePool,
          scene = this.scene;
      var _iteratorNormalCompletion = true;
    ...
    value: function startTween(key) {
      ...
      if (!config) return;
      var tween = scene.add.tween(_objectSpread$3({}, config, {
        targets: instancePool,
        onComplete: function onComplete() {
          return _this.handleOnComplete(key);
        },
        onLoop: function onLoop() {
          return _this.handleOnLoop(key);
        }
      }));
      this.instance = tween;
      return tween;
...
var allowedProps$b = ['x', 'y', 'frame', 'data'];
...
function (_DOMElement) {
  ...
  function InputText(scene) {
    ...
    for (var key in elementProperties$1) {
      var value = lodash_3(config, key, elementProperties$1[key]);
...
var allowedProps$c = ['x', 'y', 'width', 'height', 'placeholder', 'value', 'readonly', 'style', 'tooltip'];
var eventMap = {
  onInput: 'input',
  onChange: 'change',
  onDbClick: 'dblclick',
...
var PhaserRenderer = reactReconciler((_Reconciler = {
  ...
  appendChildToContainer: function appendChildToContainer(game, child) {
    if (child instanceof Phaser$1.Scene) {
      child.register(game);
    }
  },
  ...
  createInstance: function createInstance(type, props, scope) {
    switch (type) {
      ...
      case TYPES.PARTICLES:
        return new Particles$1(props);

      case TYPES.TWEEN:
        return new Tween$1(props);

      case TYPES.BLITTER:
        return new Blitter$1(props);

      case TYPES.INPUT:
        return new Input$1(props);

      default:
        return invariant_1('React-Phaser-Bindings: Unsupported component type');
    }
  },
...
function (_Phaser$Plugins$BaseP) {
  ...
  function VideoPlugin(pluginManager) {
    ...
    Phaser$1.GameObjects.Video = VideoGameObject;
    return _this;
  }
...
var BuildGameObject$1 = Phaser$1.GameObjects.BuildGameObject;
...
function (_Component) {
  ...
  _createClass(GameContext, [{
    ...
    value: function componentDidMount() {
      ...
      var game = new Phaser$1.Game(_objectSpread$5({
        type: Phaser$1.AUTO
      }, config, {
        autoStart: false,
        parent: this[tagRef],
